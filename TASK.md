...
| 渲染层主 chunk 体积裁剪：818.18KB 已超 D28 警戒 800KB（Task 1 记录基线，系 M0–M4 存量非当次增量）。构成 sourcemap 实测（2026-09-20，源码字节口径）：react-dom 最大头、CodeMirror 全家约 45%、zod 14.9%、业务代码仅 3%——首要裁剪候选为渲染层 zod 运行时（转 type-only 消费或 zod/mini），次选 manualChunks 分包、@lezer/javascript 等语言包按需 | M5 D28 按需导入红线；Task 1 评审 Important 发现（plan-mandated） | M5 Task 17 出口验收前（硬性主 chunk ≤ 800KB） | 待办 |
