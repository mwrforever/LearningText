...
| 预览外壳批次 + 预览 P1 三项（2026-09-18 终审双登记合并）：FR-SHELL-01 折叠/记忆 + FR-SHELL-02 原生菜单快捷键 + 编辑区 unsaved-guard + P1 三项——DevTools（`webContents.inspectElement`）、滚动同步（postMessage 比例+锚点协议）、CSS 热替换（协议层只读接收器注入 + postMessage 交换） | docs/superpowers/specs/2026-09-18-预览-design.md §1/§6/§7 | M4（外壳 + unsaved-guard）；P1 三项 M4/M5 | 待办 |
| 预览打磨批次：spec §4.3 resolvePath 前置反查+失败占位态、rename/move 自身 meta 刷新与 selected 同步、PreviewPanel 订阅改挂载一次（消丢广播微窗口）、text/* charset 策略评估、EditorPanel unsaved-guard 收口卸载悬挂写语义 | docs/superpowers/specs/2026-09-18-预览-design.md §4.3/§3.1/§2.3 + 终审报告 | M4 | 待办 |
