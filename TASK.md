...
| 渲染层主 chunk 体积裁剪：**988,353B（M5 Task 8 后实建，以最后一次 build 为准）已超 D28 警戒 800KB**（Task 1 基线 818,186B 系 M0–M4 存量；Task 6 shadcn 组件落地 +91.6KB：cmdk/radix-ui/cn/lucide 首入包；Task 7 全局搜索面板 +62.6KB：select 组件/搜索面板/searchModel/TreePane 插槽；Task 8 设置页 +9.3KB：@codemirror/theme-one-dark 首入包 + SettingsPage/主题联动）。构成 sourcemap 实测（2026-09-20，源码字节口径）：react-dom 最大头、CodeMirror 全家约 45%、zod 14.9%、业务代码仅 3%——裁剪候选：① 渲染层 zod 运行时（转 type-only 消费或 zod/mini）；② cn 双轨合并（cn@0.3.0 包生产消费 vs lib/cn.ts+clsx+tailwind-merge 仅测试消费——合并实现时 clsx/tailwind-merge 两依赖与 lib/cn.ts 须一并处置防死依赖）；③ manualChunks 分包、lucide 图标按需核对 | M5 D28 按需导入红线；Task 1/6 评审登记 | M5 Task 17 出口验收前（硬性主 chunk ≤ 800KB） | 待办 |
