...
- **M9 交互增强批次（六项用户指令：HTML 交互优先编辑 / 树焦点与根目录入口 / 粘贴导入 / 树内拖拽移动 / 真机 E2E 门禁 / 应用内更新；需求基线先改后动代码）**：
  - **需求基线变更（先记再改，本批动代码之前完成）**：`docs/03` §4.4 **FR-RENDER-08 修订为「交互优先」**（原文 2026-09-22 版「渲染面即编辑面：打开即整页 contentEditable」被用户实测推翻——整页可编辑使文档自带的按钮/脚本交互整体失效；新语义：默认整页保持浏览器原生交互，编辑能力经画布工具条「编辑」或双击文本元素**显式进入编辑态**、仅被选中元素就地可编辑、退出即还原纯交互）、§4.5 **FR-EDIT-05 同步修订**（编辑态语义引用 FR-RENDER-08）、§4.1 **FR-SHELL-02 二次修订**（新增 Ctrl/Cmd+V 粘贴导入加速器与标题栏更新标签）、§5 **NFR-08 修订**（网络访问收敛为「更新检查与用户确认后的更新包下载」）、**新增 NFR-12**（更新失败不影响主功能）；新增 **§4.9 M9 交互增强** 四条需求：**FR-TREE-01** 树内拖拽移动（正方形类型图标幽灵 + 合法/非法落点反馈 + 折叠目录悬停展开 + Esc 取消不留残态）、**FR-TREE-02** 目标目录选择与失焦（点空白清除目录选中、点路径条＝选中根目录）、**FR-IO-03** 粘贴导入（主进程读剪贴板文件清单直接导入，路径不出主进程）、**FR-UPDATE-01** 应用内更新（静默检查 → 标题栏更新标签 → 用户确认下载 → 重启提示）；§7.1 IPC 契约新增 `io:import-clipboard` 与 `update:get-state`/`update:check`/`update:download`/`update:install` 四通道 + `update:state` 事件广播。`docs/01` M4/M5 模块职责与边界同步修订为显式编辑态语义。
  - **决策留痕（AskUserQuestion 三问已获用户答复）**：①**路径条语义 = 选中根目录**（不在路径条上叠加「更换数据目录」入口，改数据位置仍在设置页，避免误触）；②**HTML 编辑形态 = 交互优先 + 显式编辑态**（否决「整页编辑 + 交互开关」与「编辑态加格式工具条」两案——后者超出 docs/03 现行「无标签样式设计能力」边界）；③**更新交互 = 自动检查 + 头部更新按钮标签 + 用户决定是否更新 + 下载完成提示重启进入新版**（否决全自动下载与纯手动检查两案）。
  - **实现与验收（全量门禁绿：lint/format/typecheck + 单测/集成 767 例 + coverage 门禁（全局 89.6%/83.1%，`src/main`、`src/shared`、`src/preload` 四维 100%）+ 真机 E2E 41 例）**：
    - **① HTML 画布交互优先（FR-RENDER-08 修订版）**：注入桥（`vfsProtocol.ts` PREVIEW_RECEIVER）重构——`lt:edit-enter/exit`（父→子）与 `lt:doc-edit`/`lt:edit-state`（子→父回执，双击进入/Esc 退出桥内判定）；元素级 contenteditable + `caretRangeFromPoint` 落光标 + hover/目标双属性高亮（`#64748B`=light 档 `--ring` 字面，双底达标，theme.css 同源注释）；事件闸门 capture 阶段挂卸成对（click/dblclick/submit/dragstart 阻默认，pointer/keyboard 仅阻传播）；序列化**记账还原**（(元素,属性,原值) 三元组经 `data-lt-touched` 对位克隆树，用户自带 contentEditable 原值保真——蓝图 §1.4 必修项）；交互态导航闸门最低形态（链接/表单取消就地导航，站内 `lt:link-open` 交父窗 `resolvePath` 后开新标签）。`HtmlCanvas` 单编辑会话（切签收口旧文档）、工具条双态（恒名「编辑」+ aria-pressed、编辑态实心主色换 X）、顶部提示条、`data-lt-canvas-mode` 就绪/模式锚；**首装/导航竞态修复**（真机 E2E 实证：开签即点「编辑」与首帧 onLoad 竞态曾丢编辑意图——onLoad 以 loadedIds 镜像判首装保留意图/导航注销，enter 下发加装载门）。闸门命中表施工修正：SKIP 表补 BUTTON/IMG 等表单控件与媒体元素且 tagName 大小写归一（探针实证「点按钮不退出」根因为 SKIP 缺项 + SVG 小写 tagName 永不命中）。
    - **② 树焦点与根目录入口（FR-TREE-02）**：路径条升为根目录按钮（`aria-label="根目录：<path>"`、`data-tree-node-id="1"`，选中态 aria-current；pick 模式=目标定为根）；列表空白区 pointerdown 清除树选中（非交互元素 + 非 pick 模式守卫）；Esc 键盘等价（焦点域=树 nav）。`TreePanel` 新 props（rootSelected/onClearSelection/onSelectRoot/onDropMove）。
    - **③ 粘贴导入（FR-IO-03）**：主进程 `src/main/io/clipboardFiles.ts` 读系统剪贴板文件清单（**Electron 44 同步剪贴板 API 已移除——运行时实证**，唯一入口为异步 `clipboard.read()` ClipboardItem 形态，文件复制统一映射 `text/uri-list`；FileNameW/FileName 分支保留、uri-list 为生产主路径；读取异常 warn 兜底返空）→ 新通道 `io:import-clipboard`（源路径不出主进程，不经对话框登记簿——信任边界注释钉死）→ 复用 `importNodes` 分批事务写链；响应可辨识联合 `{kind:'empty'}|{kind:'imported',result}`。渲染层 Ctrl/Cmd+V 作用域监听（仅资源树视图、非可编辑焦点、无浮层/模态/pick 模式；原生菜单**不注册 accelerator** 防劫持编辑器粘贴）+ 菜单项「粘贴导入」+ 在途守卫 ref；成功收口四件事（计数 toast/整树 stale/目标父直调回写/进度面板收口）+ 唯一文件导入即打开。
    - **④ 树内拖拽移动（FR-TREE-01）**：pointer 自制拖拽（阈值 5px/越阈 click 抑制/`body.lt-tree-dragging` 全局接管 + 画布穿透，不复用侧栏 `data-dragging` 锚）；幽灵两层 body portal（跟随层 inline transform 零过渡 + 48×48 卡片动画层）；命中判定 `elementFromPoint` → `data-tree-node-id`（新增行容器锚，与 ⋯ 触发钮 `data-node-id` 不串扰）；落点合法性纯函数 `treeModel.canDropOn`（目录/非自身/非后代/非原父级）；合法/非法/无目标三态反馈（复用 pick-target 视觉语言 + destructive 弱面）；折叠目录 600ms 驻留自动展开；drop/cancel 两路收场（180ms + fill-mode-forwards）+ pointercancel/卸载兜底；键盘等价路径=既有「移动到…」（WCAG 2.5.7）。
    - **⑤ 应用内更新（FR-UPDATE-01）**：`electron-updater@6.8.9`（精确锁定）+ `src/main/update/`（能力判定纯函数：dev/未签名 macOS/非 AppImage Linux 降级 unsupported；八态状态机服务：check deferred 幂等「以事件为准 promise 兜底」、进度钳制 0–100、`start/dispose` 定时器成对、启动 5s 首查 + 每 4h 周期；electron-updater 宽签名适配器 + 静态 import 供 vi.mock 拦截）+ 四通道 `update:get-state/check/download/install` + `update:state` 广播；渲染层标题栏三态标签（available/downloading/downloaded 才渲染）+ 设置页「关于」分区（版本/状态行/动作钮；unsupported 无钮）+「重启并安装」收口（flush 脏签 → 复用退出确认链 → 有界等待清脏 → quitAndInstall）；`electron-builder.yml` 补 publish 段（github/mwrforever/LearningText/releaseType draft——与现行「draft 出包 + 人工闸门」一致，同时生成随包 `app-update.yml`）。macOS 未签名与开发形态显式降级为 unsupported 说明（NFR-12：失败不影响主功能）。
    - **⑥ 真机 E2E（用户门禁第 5 条）**：新增 `tests/e2e/m9.spec.ts` 12 用例——面 B 三态（空白失焦/路径条选根/pick 模式 no-op）、拖拽合法移动（幽灵 + 落点高亮 + 同 id 换路径）与 Esc 取消零残态、粘贴空清单克制提示 + **真实系统剪贴板全链路**（PowerShell `Set-Clipboard` 写真实 FileDropList → Ctrl+V → 导入计数 toast + 树呈现——Electron 44 主进程无法写文件清单，探针实证后改 OS 原生通道）、画布交互态脚本可用/编辑态屏蔽 + 表单控件退出/就地编辑逐输入落库 + 序列化剥净/双击直达、更新开发形态降级呈现；既有 spec 锚点同步（title=`文档 <名>`、就绪信号 `data-lt-canvas-mode`、`body[contenteditable]` 语义收窄、编辑先经「编辑」钮、失焦提交点击点改树列表空白区）。
    - **测试与文档**：单测新增 html-canvas 3 例、panels-tree-editor 4 例、treeModel canDropOn、panels-preview-workspace paste-import、settings-page 关于 2 例、shell 更新标签 4 例、menu 粘贴导入、preload 通道 2 例、main 侧 77 例（clipboard/capability/service/adapter/ipc/app）；文档回写 docs/01（M4/M5 职责）、docs/03（§4.9 新增 + 4 处修订 + §7.1 通道 + NFR）、设计系统 §十四、布局蓝图 §2.1/§2.3/§2.5/§2.7；更新域 spec 落 `docs/superpowers/specs/2026-09-23-应用内更新-design.md`；交互蓝图落 `docs/design/2026-09-23-M9交互蓝图.md`。
