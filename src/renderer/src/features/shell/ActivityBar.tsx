...
/** 活动栏图标钮标准类串：40px 热区 + 选中态前景正色；指示条由子 span 按激活态渲染 */
const ACTIVITY_BUTTON_CLASS =
  'relative inline-flex h-9 w-9 items-center justify-center rounded-sm text-muted-foreground transition-colors duration-100 hover:bg-accent hover:text-accent-foreground aria-current:text-foreground';
    ...
    >
      ...
      >
        {view === 'tree' ? (
          <span
            aria-hidden="true"
            className="absolute top-1/2 left-0 h-5 w-0.5 -translate-y-1/2 rounded-full bg-foreground"
          />
        ) : null}
