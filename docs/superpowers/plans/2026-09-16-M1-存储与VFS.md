...
>
> **执行模式（用户 2026-09-16 已确认）：SDD（superpowers:subagent-driven-development）**——由主控会话派发子代理逐任务执行，每任务一个全新子代理 + 两阶段评审。**本会话仅定稿计划，实现全部在【新会话】执行**：加载 SDD 技能 → 读取本计划 → 从 Task 1 开始。所有工作在 **dev 分支**进行。spec 依据：`docs/superpowers/specs/2026-09-16-存储与VFS-design.md`（已定稿评审，下称「spec」）；需求基线 docs/03 §4.2/§4.3；宪法 A.4 强制生效。
